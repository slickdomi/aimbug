// Downloads the packed MaleCNS connectome and splits it into the three sparse
// matrices the hybrid model runs on (see pipeline/sim_hybrid.py for the reference).

import type { BrainData, Meta, WorkerIn, WorkerOut } from "./types";

const post = (msg: WorkerOut, transfer: Transferable[] = []) =>
  (self as unknown as Worker).postMessage(msg, transfer);

async function fetchBytes(url: string, label: string, expected: number, gz: boolean, base = 0): Promise<Uint8Array> {
  ...
  for (;;) {
    ...
    post({ kind: "progress", label, done: base + got, total: expected });
  }
...
async function load(req: WorkerIn): Promise<BrainData> {
  ...
  const nb = await fetchBytes(`${req.url}/neurons.bin.gz`, "neurons", 0, true);
  ...
  for (const sh of meta.shards) {
    const buf = await fetchBytes(`${req.url}/${sh.file}`, "downloading synapses", totalBytes, true, downloaded);
    downloaded += sh.bytes;
  ...
  post({ kind: "progress", label: "wiring the optic lobe", done: 0, total: 1 });
  ...
  for (let pre = 0; pre < n; pre++) {
    ...
    for (let k = offsets[pre]; k < offsets[pre + 1]; k++) {
      ...
      } else if (q >= ng && !modulatory[nt[pre]]) nSpike++;
    }
  }
  ...
  for (let pre = 0; pre < n; pre++) {
    ...
    for (let k = offsets[pre]; k < offsets[pre + 1]; k++) {
      ...
      } else if (q >= ng && !modulatory[nt[pre]]) {
        spikeEdges[s++] = (q | (((c * sg) & 0x3fff) << 18)) >>> 0;
      }
    }
  }
  spikeOffsets[n] = s;
  ...
  return {
    ...
    stats: { spikeEdges: s, gradedEdges: gradedOffsets[ng], ifaceEdges: ifaceOffsets[n - ng] },
  };
}

self.onmessage = async (ev: MessageEvent<WorkerIn>) => {
  try {
    const d = await load(ev.data);

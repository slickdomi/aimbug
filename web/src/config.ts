// Model constants. These mirror pipeline/sim_hybrid.py, where they were validated
...
export const MODEL = {
  ...
  // antennal-lobe LNs) lock at max rate after any perturbation.
  adapt: 2.0,
  tauAdapt: 200,
  ...
  // above ~2.39 make them slowly unstable (pipeline/graded_stability.py).
  gradedGain: 2.3,
  gradedMin: -1,
  gradedMax: 4,
...
export const GAME = {
  ...
  // Pitch: DNp53 (both hemispheres) is the only descending neuron that tracks target elevation
  // (pipeline/pitch_scan.py; in game ~0-0.5 Hz below, 2-4 Hz above). Nothing prefers targets
  // below, so it is balanced against LC4+LPLC2, which report "a target is visible" at any
  // elevation: pitch rate = gain * (DNp53 - pitchRef * LC) - spring * pitch.
  ...
  songThreshold: 15, // Hz, pIP10 left + right, slow-filtered (~0.5 shots/s in smoke trials)
